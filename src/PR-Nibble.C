...
struct ACL_F {
	...
	/* constructor */
	ACL_F(sparseAdditiveSet<float> &_p, sparseAdditiveSet<float> &_r,
		sparseAdditiveSet<float> &_new_r, vertex* _V,
		double _oneMinusAlphaOverTwo) : 
		p(_p), r(_r), new_r(_new_r), V(_V),
		oneMinusAlphaOverTwo(_oneMinusAlphaOverTwo) {}

	/* UpdateNeighbors */
	inline bool update(uintE s, uintE d)
	{
		// (1-a)r[s]/(2d(s))
		return new_r.insert(make_pair(d,
				oneMinusAlphaOverTwo*r.find(s).second/V[s].getOutDegree()));
	}
	/* UpdateSelf */
...
struct Local_Update {
	...
	Local_Update(sparseAdditiveSet<float> &_p, sparseAdditiveSet<float> &_r,
		double _alpha) :
